...
import { addToCart } from './CartSlice.jsx'; // Import addToCart action
...
function ProductList({ onHomeClick }) {
    ...
    const handleAddToCart = (plant) => {
        console.log('Adding plant to cart:', plant); // Debug log
        
        // Dispatch the addToCart action with the plant details
        dispatch(addToCart({
            id: plant.id,
            name: plant.name,
            image: plant.image,
                            ...
                            <div className="product-list">
                                {category.plants.map((plant, plantIndex) => (
                                    <div className="product-card" key={plant.id}>
                                        ...
                                        <button
                                            className={`product-button ${isInCart(plant.id) ? 'added-to-cart' : ''}`}
                                            onClick={() => handleAddToCart(plant)}
                                            disabled={isInCart(plant.id)}
                                            style={{
                                                backgroundColor: isInCart(plant.id) ? '#gray' : '#4CAF50',
                                                opacity: isInCart(plant.id) ? 0.6 : 1,
